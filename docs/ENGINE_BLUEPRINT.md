...
Рабочее хранилище — SQLite (`data/cms.sqlite`). Legacy-файлы `data/*.json` используются только для первичного импорта. Cookie-сессии, хеш пароля администратора и счётчики неудачных попыток входа также хранятся в SQLite.
